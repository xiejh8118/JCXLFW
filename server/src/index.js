...
const { match, localizeName } = require('./services/matchService');
...
function resolveMatched(ids, lang) {
  ...
  return db.PROVIDERS.filter(p => ids.includes(p.id)).map(p => ({
    id: p.id, name: localizeName(p, lang), kind: p.kind, contact: p.contact, rating: p.rating
  }));
}
...
async function handle(req, res, url, body) {
  ...
  if (p === '/api/ai/chat' && req.method === 'POST') {
    try {
      ...
      const result = await chat({ messages: messages || [], lang: lang || 'zh-CN' });
      return send(res, 200, { code: 0, data: result });
    } catch (e) { return send(res, 500, { code: 500, message: e.message }); }
  }
  ...
  if (p === '/api/match' && req.method === 'POST') {
    try {
      const { type, query, lang } = body || {};
      const list = match(type, query, lang || 'zh-CN', 5);
